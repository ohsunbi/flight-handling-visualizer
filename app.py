...
# departures block: base + extra (dashed)
dep_block = pd.concat([
...
for i, row in dep_block.iterrows():
    ls = "--" if "EXTRA" in row["type"] else "-"
    ax1.plot([row["start"], row["end"]], [i, i], color="blue", linewidth=4, linestyle=ls, label="Departure" if (i==0 and ls=="-") else ("Departure (extra)" if (i==0 and ls=="--") else ""))
    if row["Label"]:
...
# arrivals block: base + extra (dashed), plotted below departures with offset 0.4
arr_block = pd.concat([
...
for i, row in arr_block.iterrows():
    y = i + 0.4
    ls = "--" if "EXTRA" in row["type"] else "-"
    ax1.plot([row["start"], row["end"]], [y, y], color="red", linewidth=4, linestyle=ls, label="Arrival" if (i==0 and ls=="-") else ("Arrival (extra)" if (i==0 and ls=="--") else ""))
    if row["Label"]:
...
# build combined dep and arr windows
...
# common x-range
start_time = min(
    dep_intervals["start"].min() if len(dep_intervals)>0 else pd.Timestamp(base_date),
    arr_intervals["start"].min() if len(arr_intervals)>0 else pd.Timestamp(base_date)
)
end_time = max(
    dep_intervals["end"].max() if len(dep_intervals)>0 else pd.Timestamp(base_date) + timedelta(hours=23, minutes=59),
...
# annotate counts; hide y-axis to align x with top chart
